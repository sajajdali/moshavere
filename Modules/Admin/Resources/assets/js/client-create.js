...
function selectClient (client) {
    ...
    var $client = $(
      '<span><img src="https://cdn.enabz.org/assets/admin/images/users/' +  client.element.value.toLowerCase() + '.jpg" class="rounded-circle avatar-sm" /> '
       + client.text +  '</span>'
   );
   return $client;
};
...
function selectCountry (country) {
    ...
    if(country.element.value.toLowerCase() !== 'def'){
        var $country = $(
            '<span><img src="https://cdn.enabz.org/assets/admin/images/flags/' +  country.element.value.toLowerCase() + '.svg" class="br-5 avatar-sm" /> '
            + country.text +  '</span>'
       );
       return $country;
    }

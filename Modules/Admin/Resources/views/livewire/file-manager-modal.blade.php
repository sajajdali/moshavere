<div>
    <!-- modal -->
    <div class="modal fade @if ($isDialogShow) show @endif" data-bs-backdrop="static"
                                ...
                                @foreach ($broadcamp as $indexItem => $item)
                                    ...
                                    @else
                                        <li class="breadcrumb-item">
                                            <button wire:click="goTo({{ $indexItem }})">{{ $item['title'] }}</button>
                                        </li>
                                    @endif
                                @endforeach
                            </ol>
                    ...
                    @if ($this->listFiles()->count() > 0)
                        <div class="table-responsive mb-3">
                            <table class="table text-nowrap text-md-nowrap table-bordered" wire:loading.class="op-0-3">
                                ...
                                <tbody>
                                    @foreach ($this->listFiles() as $item)
                                        @if ($item['type'] == 'folder')
                                            <tr style="cursor: pointer"
                                                wire:click="goToDirectory('{{ $item['name'] }}')">
                                                <td>{{ $loop->iteration }}</td>
                                                <td colspan="2"><img
                                                        src="{{ admin_asset('images/files/folder.png') }}"
                                                        alt="img" style="width: 36px">
                                                    {{ $item['name'] }}
                                                </td>
                                            </tr>
                                        @else
                                                        ...
                                                        <div class="d-flex align-items-center">
                                                            ...
                                                            <div>
                                                                <h6 class="mb-1">{{ $item['name'] }}
                                                                    ...
                                                                    class="text-muted fs-13">{{ $item['size'] }}</span>
                                                            </div>
                                                        </div>
                                                    </a>
                                                </td>

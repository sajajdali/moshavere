        ...
        <section class="pb-12">
            <div class="container space-y-8">
                <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
                    <div class="space-y-2">
                        <h3 class="font-bold text-xl"> {{setting(\Modules\Setting\Enum\SettingKeyEnum::SITE_SECEND_SECTION_TITLE) ?? 'معرفی پزشکان'}} </h3>
                        <p class="text-secondary-500">  {{setting(\Modules\Setting\Enum\SettingKeyEnum::SITE_SECEND_SECTION_DESCRIPTION) ?? 'ارتباط با پزشکانی که در سریع‌ترین زمان امکان پاسخگویی دارند'}} </p>
                    </div>
                        ...
                        @foreach ($fetchData['introductionDoctors'] as $introDoc)
                            <div class="swiper-slide">
                                <article role="banner"
                                    ...
                                    <button wire:click='docpage({{$introDoc->id}})'
                                        class="btn__blue--tint">
                                        <div role="status" wire:loading wire:target='docpage({{$introDoc->id}})'>
                                    ...
                                    <span wire:loading.remove wire:target='docpage({{$introDoc->id}})' >دریافت نوبت</span>
                                    </button>
                                </article>
                            </div>
                        @endforeach

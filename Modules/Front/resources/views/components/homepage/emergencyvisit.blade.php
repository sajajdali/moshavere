        ...
        <section class="py-12">
            <div class="container space-y-8">
                <div class="flex flex-col md:flex-row itemsstart md:items-center justify-between gap-4">
                    <div class="space-y-2">
                        <h3 class="font-bold text-xl text-green"> {{setting(\Modules\Setting\Enum\SettingKeyEnum::SITE_FIRST_SECTION_TITLE) ?? 'ویزیت فوری پزشک'}} </h3>
                        <p class="text-secondary-500"> {{setting(\Modules\Setting\Enum\SettingKeyEnum::SITE_FIRST_SECTION_DESCRIPTION) ?? 'ارتباط با پزشکانی که در سریع‌ترین زمان امکان پاسخگویی دارند' }} </p>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    @foreach ($fetchData['EmergencyDoctors'] as $emDoc)
                        <article role="banner"
                            ...
                            <button wire:click='docpage({{$emDoc->id}})' type="button"
                                class="btn__green">
                                ...
                                <span wire:loading.remove wire:target="docpage({{$emDoc->id}})">دریافت نوبت</span>
                            </button>
                        </article>
                    @endforeach
                </div>

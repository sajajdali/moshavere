<div>
    <!-- header -->
    <header class="bg-primary-main pt-16 pb-8 space-y-10 md:space-y-16 main-slider-with-logo">
        <section class="w-full max-w-[770px] mx-auto px-5 flex flex-col items-center gap-6 md:gap-8">
            <h1 class="text-2xl md:text-3xl text-center leading-[2.5rem] font-bold text-white">
                {{ setting(Modules\Setting\Enum\SettingKeyEnum::SITE_SLIDER_TITLE) }}</h1>
            @if(setting(\Modules\Setting\Enum\SettingKeyEnum::UI_NOW_SHOW_SEARCH_BAR) !== '1')
                <form wire:submit='searchFor'
                      class="flex items-center p-3 w-full relative bg-white rounded-3xl border-4 md:border-8 border-blue-sky">
                    <input type="text" wire:model='form.searchProp'
                           class="text-sm md:text-base flex-grow border-none outline-none pr-7 md:pr-12"
                           placeholder="جستجوی پیشرفته"/>
                    <svg
                        class="absolute top-1/2 -translate-y-1/2 right-3 md:right-5 text-gray-700 w-5 h-5 md:w-6 md:h-6"
                        xmlns="http://www.w3.org/2000/svg">
                        <use xlink:href="#sprite-search"/>
                    </svg>
                    <button type="submit" class="btn__blue">
                        <div role="status" wire:loading wire:target='searchFor'>
                            <svg aria-hidden="true"
                                 class="w-5 h-5 text-white-200 animate-spin dark:text-white-600 fill-gray-700"
                                 viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path
                                    d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
                                    fill="currentColor"/>
                                <path
                                    d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
                                    fill="currentFill"/>
                            </svg>
                            <span class="sr-only">Loading...</span>
                        </div>
                        <span wire:loading.remove wire:target='searchFor'>جست و جو</span>
                    </button>
                </form>
            @endif
        </section>
        <section class="container">
            <div class="flex flex-col md:flex-row gap-5 items-center justify-between rounded-xl bg-white py-4 px-5">
                <div class="flex items-center gap-4">
                    <div class="w-[40px] h-[40px] bg-primary-tint-100 rounded-full flex items-center justify-center">
                        <svg class="text-primary-main w-6 h-6" xmlns="http://www.w3.org/2000/svg">
                            <use xlink:href="#sprite-time"/>
                        </svg>
                    </div>
                    <div class="w-[calc(100%-40px-1rem)] md:w-auto space-y-2">
                        <h2 class="text-xl font-semibold">دریافت نوبت حضوری</h2>
                        <p>دریافت نوبت اینترنتی برای مراجعه حضوری به مطب پزشکان</p>
                    </div>
                </div>
                <a href="{{ route('front.searchPage', ['query' => 'پزشکان']) }}"
                   class="flex items-center gap-3 text-primary-main">
                    <span class="font-semibold">مشاهده لیست پزشکان</span>
                    <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg">
                        <use xlink:href="#sprite-arrow-left"/>
                    </svg>
                </a>
            </div>
        </section>
        <section class="w-full max-w-[770px] mx-auto px-5 flex flex-col items-center gap-6 md:gap-8">
            <div class="flex flex-col md:flex-row gap-5 items-center justify-between rounded-xl bg-white py-4 px-5">
                <a href="{{ route('front.searchPage', ['query' => 'پزشکان']) }}"
                   class="text-2xl md:text-3xl text-center leading-[2.5rem] text-xl font-semibold">
                    دریافت نوبت
                </a>
            </div>
        </section>
    </header>
</div>

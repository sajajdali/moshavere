<div class="mt-5">
        ...
        @if (!$isPaymentDone)
            <div class="col-md-12 d-flex flex-column">
                <div class="d-flex justify-content-between align-items-center mx-2">
                    ...
                    @if ($fetch['expired_time'])
                        <h5 class="badge bg-secondary rounded-pill"> {{ $fetch['expired_time'] }} </h5>
                    @else
                        <h5 class="badge bg-danger rounded-pill"> منقضی شده</h5>
                    @endif
                </div>
            </div>
        @endif
                ...
                @if (!$isPaymentDone)
                    <div class="card-header  border-bottom d-flex justify-content-between align-items-center">
                        ...
                        <h4 class="d-flex flex-column">
                            ...
                            @endif
                            <span>
                                {{ number_format($transaction->total_cost) }} تومان
                            </span>
                        </h4>
                    </div>
                @endif
                    ...
                    @if (!$isPaymentDone)
                        <div class="row">
                            ...
                            @if (setting(Modules\Setting\Enum\SettingKeyEnum::PAYMENT_RULES_AND_CONDITION_STATUS) &&
                                    setting(Modules\Setting\Enum\SettingKeyEnum::PAYMENT_RULES_AND_CONDITION_STATUS) != false)
                ...
                <div class="card-fotter">
                    <div class="row my-5 mx-5">
                        <div class="col-12">
                            @if (!$isPaymentDone)
                                @if ($fetch['expired_time'])
                                    <button id="paymentButton" wire:click='createZarinPalReqeust'
                                        wire:target='createZarinPalReqeust' wire:loading.class='btn-loading'
                                        class="btn btn-primary w-100 ">پرداخت و فعال سازی
                                        نوبت</button>
                                @else
                                    <button id="paymentButton" class="btn btn-danger w-100 " disabled> مهلت پرداخت به
                                        پایان
                                        رسیده</button>
                                @endif
                            @else
                                <button id="paymentButton" class="btn btn-success w-100 " disabled> پرداخت انجام شده است
                                </button>
                            @endif
    ...
    @if (isset($fetch['appointmentUser']))
        <div class="row">
            <div class="col-lg-12">
                <div class="card custom-card rounded-20">
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-10 d-flex flex-column">
                                ...
                                <span>{{ $fetch['appointmentUser']->doctor->fullname }}</span>
                                <span class="text-gray">
                                    @foreach ($fetch['appointmentUser']->doctor->specialities as $speciality)
                                        {{ $speciality->title }}
                                        @if (!$loop->last)
                                            ,
                                        @endif
                        ...
                        <div class="row">
                            ...
                            <div class="col-10 d-flex flex-column">
                                ...
                                <span>{{ verta($fetch['appointmentUser']->date_visit)->format('F d') }} </span>
                                <span class="text-gray"> ساعت
                                    {{ verta($fetch['appointmentUser']->date_visit)->format('H:i') }} </span>
                            </div>
                            <div class="col-12">
                        ...
                        <div class="row">
                            ...
                            <div class="col-10 d-flex flex-column">
                                ...
                                <span>{{ $fetch['appointmentUser']->user->fullname ?? '-' }} </span>
                                <span class="text-gray"> شماره پرونده:
                                    {{ $fetch['appointmentUser']->user->document_number ?? '-' }}</span>
                            </div>
                            <div class="col-12">
                                <hr class="bg-gray opacity-25 mx-3">
                            </div>
            ...
            $.each(sentences, function(index, sentence) {
                if (sentence.trim() !== '') {
                    ...
                    descriptionContainer.append(
                        '</br><i class="fa fa-info-circle me-2" aria-hidden="true"></i>' +
                        sentence.trim() + '. ');
                }
            });

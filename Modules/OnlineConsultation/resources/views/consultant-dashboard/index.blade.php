...
@forelse($rows as $row)
    ...
    <a class="oc-panel oc-consultant-card" href="{{ route('admin.consultation.consultants-dashboard.show', $mode==='monthly' ? ['practitioner'=>$p,'period'=>'month','month'=>$selectedMonth] : ['practitioner'=>$p,'period'=>'custom','from'=>$selectedDate,'to'=>$selectedDate]) }}">
        ...
        <div class="oc-consultant-metrics">
            @foreach([['کل نوبت',$s['total'],''],['انجام‌شده',$s['completed'],'good'],['در انتظار',$s['remaining'],''],['ازدست‌رفته',$s['missed'],'danger'],['تماس‌ها',$s['calls'],''],['پاسخ‌داده‌شده',$s['answered'],'good'],['بی‌پاسخ',$s['unanswered'],'danger'],['مدت مکالمه',$duration($s['talk_seconds']),''],['مبلغ مشاوره',number_format($s['income']).' تومان',''],['بازگشت‌داده‌شده',number_format($s['refunded']).' تومان','']] as [$label,$value,$tone])

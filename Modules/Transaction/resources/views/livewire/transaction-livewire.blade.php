                                    ...
                                    @if ($transactions->isNotEmpty())
                                        @foreach ($transactions as $transaction)
                                            <tr class="text-center {{ $transaction->status->rowClassColor() }}">
                                                ...
                                                <td>
                                                    <a target="bank"
                                                        href="{{ route('admin.user.document', ['user' => $transaction->user->id]) }}">
                                                        {{ $transaction->user->fullName }}
                                                    </a>
                                                </td>
                                                <td>
                                                    <a
                                                        href="{{ route('admin.appointment_user.list', ['search' => ['appointment_id' => $transaction->appid()]]) }}">
                                                        {{ $transaction->appid() }}
                                                    </a>
                                                </td>
                                                <td>{{ verta($transaction->created_at)->format('%d %b، ساعت H:i') }}
                                                </td>
                                                <td>{{ verta($transaction->updated_at)->format('%d %b، ساعت H:i') }}
                                                </td>
                                                <td>
                                                ...
                                                <td>
                                                    <span class="cart_hash"
                                                        data-id="{{ $transaction->getCartHash() }}"
                                                        style="cursor: pointer">
                                                        {{ substr($transaction->getCartHash(), 0, 10) }}
                                                    </span>
                                                </td>
                                                <td>
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            $('body').on('click', '.cart_hash', function() {
                var cartHash = $(this).data('id');

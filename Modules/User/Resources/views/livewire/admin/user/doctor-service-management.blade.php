            ...
            @if ($activeTab === 'doctors')
                <div class="row g-3 mb-5 align-items-end">
                    ...
                    <div class="col-lg-4 col-md-6">
                        ...
                        <input id="doctor-last-name" type="text" class="form-control" placeholder="جست‌وجو با نام خانوادگی"
                            wire:model.live.debounce.400ms="search.last_name">
                    </div>
                    ...
                    <div class="col-lg-1 col-md-6">
                        <button type="button" class="btn btn-secondary w-100" wire:click="resetDoctorSearch" title="پاک کردن جست‌وجو">
                            ...
                            <div class="card h-100 mb-4 shadow-sm {{ $hasLicence ? 'border' : 'border border-danger' }}"
                                ...
                                <div class="card-body d-flex flex-column">
                                    <div class="d-flex align-items-center mb-3">
                                        <img src="{{ $doctor->getUserAvatar() }}" class="avatar avatar-lg brround me-3" alt="{{ $doctor->full_name }}">
                                        <div>
                                    ...
                                    <div class="mb-4 flex-grow-1">
                                        ...
                                        @forelse ($doctor->services as $service)
                                            <span class="badge bg-primary-transparent text-primary me-1 mb-1">{{ $service->title }}</span>
                                        @empty
                                            <span class="text-muted">بخشی ثبت نشده است</span>
                                        @endforelse
                                    </div>

                                    <label class="form-label" for="licence-{{ $doctor->id }}">شماره نظام پزشکی</label>
                                    <div class="input-group">
                                        <input id="licence-{{ $doctor->id }}" type="text"
                                            ...
                                            @endcan>
                                        @can('update', $doctor)
                                            <button type="button" class="btn btn-primary" wire:click="saveLicence({{ $doctor->id }})"
                                                wire:loading.attr="disabled" wire:target="saveLicence({{ $doctor->id }})">
                                                ذخیره
                                            </button>
                                        @endcan
                                    </div>
